...
import { catalog, featuredStories, searchCatalog, trendingBoards } from './catalog.js';
import { resolveStoredTheme } from './theme.js';
import NewsView from './views/NewsView.vue';
import DirectoryView from './views/DirectoryView.vue';
import RankingView from './views/RankingView.vue';

/** Primary navigation destinations for the complete discovery routes. */
const navigation = [
  { label: '资讯', target: 'news', route: '/news' },
  { label: '工具', target: 'tools', route: '/tools' },
  { label: '开源', target: 'opensource', route: '/opensource' },
  { label: '榜单', target: 'ranking', route: '/ranking' },
];
...
/** Current Vue route used to switch between the landing page and the complete news destination. */
const route = useRoute();
/** Router used for explicit page-level navigation from the landing-page shell. */
const router = useRouter();
/** Compact daily GitHub ranking shown only as a landing-page preview. */
const featuredRanking = trendingBoards.GitHub.daily.slice(0, 3);
...
const displayedResources = computed(() => {
  return visibleResources.value.slice(0, 3);
});
...
/**
 * Uses a real page route for the complete news feed, while preserving smooth section navigation on the landing page.
 * @param {{ target: string, route?: string }} item Primary-navigation configuration.
 * @returns {void}
 */
...
/**
 * Opens the complete news feed as a page rather than reusing an unrelated landing-page anchor.
 * @returns {void}
 */
...
/**
 * Opens a complete directory page from a compact landing-page preview.
 * @param {'/tools' | '/opensource' | '/ranking'} destination Reader-selected directory route.
 * @returns {void}
 */
function openDirectory(destination) {
...
<template>
  <NewsView v-if="route.name === 'news'" />
  <DirectoryView v-else-if="route.name === 'tools'" kind="工具" title="AI 工具目录" eyebrow="AI TOOL DIRECTORY" description="按写作、图像、开发和模型能力筛选主流 AI 工具。每项都附有简明判断与官方入口。" />
  <DirectoryView v-else-if="route.name === 'opensource'" kind="开源" title="开源项目目录" eyebrow="OPEN SOURCE DIRECTORY" description="从 MCP、Agent 到本地模型与多模态工作流，整理可直接上手的开源项目与仓库。" />
  <RankingView v-else-if="route.name === 'ranking'" />
  <template v-else>
  ...
  <main id="top">
    ...
    <section id="news" class="content-band top-stories news-section" aria-labelledby="news-title">
      <div class="wrap shell">
        <div v-reveal class="section-head section-heading">
          <div><p class="eyebrow">TOP STORY</p><h2 id="news-title">值得先看的内容</h2></div>
          <button class="text-link focus-ring" type="button" @click="openNewsFeed">浏览全部资讯 <span aria-hidden="true">→</span></button>
        </div>
        <div class="story-grid editorial-grid">
          <article v-reveal class="feature-card feature-story surface-transition">
            <span class="feature-label">本周焦点 · {{ featuredStories[0].type }}</span>
            <h3>{{ featuredStories[0].title }}</h3>
            <p>{{ featuredStories[0].summary }}</p>
          </article>
          <aside class="rail breaking-rail" aria-label="正在发生的资讯">
            <div class="rail-head breaking-head"><b>正在发生</b><button class="text-link focus-ring" type="button" @click="openNewsFeed">全部资讯 <span aria-hidden="true">→</span></button></div>
            <article v-for="(story, index) in featuredStories.slice(1)" :key="story.title" v-reveal class="rail-card breaking-card surface-transition" :style="{ '--reveal-delay': `${index * 100}ms` }">
    ...
    <section id="tools" class="content-band tools tools-section" aria-labelledby="tools-title">
      <div class="wrap shell">
        <div v-reveal class="section-head section-heading section-heading--resources">
          ...
          <button class="text-link focus-ring" type="button" @click="openDirectory('/tools')">浏览全部工具 <span aria-hidden="true">→</span></button>
        </div>
        <div v-reveal class="category-bar" role="tablist" aria-label="工具场景分类">
          ...
          <button v-for="item in displayedResources" :key="item.id" v-reveal class="tool-card resource-card surface-transition focus-ring" type="button" :aria-label="`查看 ${item.name} 详情`" @click="openResource(item)">
            ...
            <div class="resource-directory-action"><button class="directory-toggle focus-ring" type="button" @click="openDirectory('/tools')">查看完整目录 <span aria-hidden="true">→</span></button></div>
          </div>
        </section>
    ...
    <section id="opensource" class="content-band open-source-section" aria-labelledby="opensource-title">
      <div class="wrap shell split-layout">
        <div v-reveal class="open-source-copy"><p class="eyebrow">OPEN SOURCE WATCH</p><div class="open-source-title-row"><h2 id="opensource-title">本周值得上手的开源项目</h2><span aria-hidden="true">04 / SIGNAL</span></div><p>关注可复用的 Skill、可连接的 MCP、可落地的 Agent 框架与本地模型工具。每个项目都以清楚的用途和热度信号呈现。</p><button class="text-action focus-ring" type="button" @click="openDirectory('/opensource')">浏览完整开源目录 <span aria-hidden="true">→</span></button></div>
        <div v-reveal class="project-list">
          <button v-for="(item, index) in catalog.filter((resource) => resource.kind === '开源').slice(0, 3)" :key="item.id" class="project-row focus-ring" type="button" :aria-label="`查看 ${item.name} 详情`" @click="openResource(item)"><span class="project-rank">0{{ index + 1 }}</span><span class="project-name">{{ item.name }}</span><span class="project-description">{{ item.description }}</span><span class="project-metric">{{ item.metric }}</span><span aria-hidden="true">↗</span></button>
        </div>
      </div>
    </section>
    ...
    <section id="ranking" class="content-band ranking-section" aria-labelledby="ranking-title">
      <div class="wrap shell ranking-layout">
        <div v-reveal class="section-heading"><div><p class="eyebrow">OPEN SOURCE TRENDING</p><h2 id="ranking-title">GitHub 日榜精选</h2></div><button class="text-link focus-ring" type="button" @click="openDirectory('/ranking')">查看完整榜单 <span aria-hidden="true">→</span></button></div>
        <div class="ranking-grid">
          <a v-for="(project, index) in featuredRanking" :key="project.name" v-reveal class="ranking-card surface-transition focus-ring" :href="project.url" target="_blank" rel="noreferrer"><p class="ranking-index">{{ String(index + 1).padStart(2, '0') }}</p><p class="eyebrow">{{ project.language }}</p><h3>{{ project.name }}</h3><p>{{ project.description }}</p><span>{{ project.change }} stars</span></a>
        </div>
      </div>
    </section>
  </main>
